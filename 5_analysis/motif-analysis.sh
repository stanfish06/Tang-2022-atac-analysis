...
generate_targets_bed() {
	...
	local genes_pattern
	genes_pattern=$(printf '%s\n' "${genes[@]}" | paste -sd'|')

	if [[ "$gtf" == *.gz ]]; then
		zcat "$gtf"
	else
		cat "$gtf"
	fi | awk -v pattern="$genes_pattern" -v window="$window" '
		BEGIN { OFS="\t" }
		$3 == "gene" {
			match($0, /gene_name "([^"]+)"/, m)
			gene = m[1]
			if (gene !~ "^(" pattern ")$") next
			...
			label = gene "_" window/1000 "kb"
			print chr, start, end, label
		}
	' | sort -k1,1 -k2,2n >"$out"

	echo "Generated $out with $(wc -l <"$out") target regions"
}

echo "Generating targets BED from GTF..."
generate_targets_bed "$REF_GTF" "$TARGETS_BED" "$TSS_WINDOW" "${TARGET_GENES[@]}"
